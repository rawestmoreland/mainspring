...
function RouteComponent() {
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground font-mono">Loading watch…</div>;
  }
  ...
  const displayedPhotos = stageFilter === 'all' ? photos : photos.filter((ph) => ph.stage === stageFilter);
            ...
            <div className="flex gap-1.5 px-4 py-3 border-t border-border overflow-x-auto">
              {displayedPhotos.map((ph, i) => (
                ...
                >
                  ...
                  {user && (
                    <div
                      onClick={(e) => { e.stopPropagation(); deletePhoto.mutate(ph.id); }}
        ...
        <div className="flex-1 space-y-5 min-w-0">
          ...
          <section className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="px-4 py-2.5 border-b border-border">
              <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Details</span>
            </div>
            {[
          ...
          <section className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
              ...
              {user && !editingNotes && (
                <button
                  onClick={() => { setDraftNotes(watch.notes ?? ''); setEditingNotes(true); }}
              ...
              {editingNotes ? (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => { updateWatch.mutate({ ...watch, notes: draftNotes }); setEditingNotes(false); }}
                ...
                <div className="prose max-w-none" dangerouslySetInnerHTML={{ __html: watch.notes }} />
              ) : (
                <p className="font-mono text-xs italic text-muted-foreground/50">No notes yet.</p>
              )}
            </div>
          </section>
        </div>
