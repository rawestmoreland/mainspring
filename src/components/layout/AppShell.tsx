...
export function AppShell({ children }: { children: ReactNode }) {
  const { watches, selectedWatch, closeWatch, updatePhotos } = useWatches();
    ...
    <div className="flex min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <aside className="w-[220px] shrink-0 bg-zinc-900 border-r border-zinc-800 flex flex-col py-7 sticky top-0 h-screen">
        ...
        <div className="px-6 pb-7 border-b border-zinc-800 mb-5">
          <h1 className="font-serif text-lg font-bold text-amber-500 leading-tight">Mainspring</h1>
              ...
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ...
                    : 'text-zinc-500 border-l-transparent hover:text-zinc-200 hover:bg-white/[0.03]',
                )}
              >
        ...
        <div className="px-5 pt-5 border-t border-zinc-800">
          <div className="bg-amber-500/10 border border-amber-700/40 rounded-md p-3">
            ...
            <div className="font-serif text-sm font-semibold text-amber-400">{GOAL_LABEL}</div>
      ...
      <main className="flex-1 overflow-y-auto min-w-0">
        ...
        <div className="flex items-end justify-between px-9 pt-8 pb-5 border-b border-zinc-800">
          <div>
            ...
            <p className="font-mono text-[11px] text-zinc-500 tracking-widest mt-1">{subtitle}</p>
          </div>
          {(pathname === '/' || pathname === '/watches') && (
            <button
              className="rounded font-semibold tracking-wide transition-opacity hover:opacity-80 cursor-pointer bg-amber-600 text-zinc-950 px-4 py-2 text-xs"
            >
              + Add Watch
            </button>
          )}
      ...
      {/* Expose selectWatch on window for child route usage (avoids prop drilling) */}
      {/* Routes use useWatches() hook directly instead */}
    </div>
  );
}
