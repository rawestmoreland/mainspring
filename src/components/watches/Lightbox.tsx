    ...
    >
      <div className='relative' onClick={(e) => e.stopPropagation()}>
        ...
        {photos.length > 1 && (
          <button
            onClick={prev}
            className='absolute -left-14 top-1/2 -translate-y-1/2 bg-white/8 hover:bg-white/15 text-white text-2xl px-3.5 py-2.5 rounded border-none cursor-pointer transition-colors'
          >
            ‹
          </button>
        )}
        ...
        {photos.length > 1 && (
          <button
            onClick={next}
            className='absolute -right-14 top-1/2 -translate-y-1/2 bg-white/8 hover:bg-white/15 text-white text-2xl px-3.5 py-2.5 rounded border-none cursor-pointer transition-colors'
          >
            ›
          </button>
        )}
